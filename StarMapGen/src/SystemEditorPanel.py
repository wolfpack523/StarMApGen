...
class SystemEditorPanel(wx.Panel):
    ...
    def createControls(self):
        ...
        jump_button_sizer = wx.BoxSizer(wx.HORIZONTAL)
...
"""
    ...
    def refreshJumpEditor(self, system_name):
        ...
        for jump in self.jumpList:
            if not jump.contains(system_name):
                continue

            other_name = jump.getOtherSystemName(
                system_name
            )
    ...
    def updateJumpButtons(self):
        system_name = self.getCurrentSystemName()
        ...
        self.addJumpButton.Enable(can_add)
        self.editJumpButton.Enable(can_edit)
        self.removeJumpButton.Enable(can_edit)
    ...
    def getSelectedJumpLink(self):
        ...
        if (
                selected_row == -1
                or not 0 <= selected_row < len(
            self.displayedJumpLinks
        )
        ):
            return None

        return self.displayedJumpLinks[selected_row]
    ...
    def getAvailableJumpTargets(
        ...
        connected_names = {
            jump.getOtherSystemName(system_name)
            for jump in self.jumpList
            if jump.contains(system_name)
        }
        ...
        return sorted(
            [
                ...
                if (
                    ...
                    and (
                            system.name == current_target
                            or system.name not in connected_names
                    )
            )
            ],
            key=str.casefold,
    ...
    def addJumpLink(self, event):
        ...
        target_names = self.getAvailableJumpTargets()
        ...
        try:
            ...
            target_name = dialog.getTargetName()
        ...
        self.saveAndRedrawCurrentMap()
        self.refreshJumpEditor(system_name)

        self.SetStatusText(
            f'Jump link to "{target_name}" was added.'
        )
    ...
    def editJumpLink(self, event):
        ...
        if system_name is None or jump is None:
            return

        current_target = jump.getOtherSystemName(
            system_name
        )
        ...
        self.SetStatusText(
            f'Jump link to "{target_name}" was updated.'
        )
    ...
    def removeJumpLink(self, event):
        ...
        target_name = jump.getOtherSystemName(
            system_name
        )

        result = wx.MessageBox(
            f'Remove the jump link to "{target_name}"?',
            "Remove Jump Link",
            wx.YES_NO
            | wx.NO_DEFAULT
            | wx.ICON_QUESTION,
        ...
        self.SetStatusText(
            f'Jump link to "{target_name}" was removed.'
        )
